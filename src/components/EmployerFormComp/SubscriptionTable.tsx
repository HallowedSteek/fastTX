...
const SubTable: FC<Sub> = ({
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      setCountdown(formatDuration(duration, {format: ["days", "hours", "seconds"], zero: true}));
    }, 1000);
  ...
  useEffect(() => {
    if (
      new Date().getTime() > new Date(parseInt(subscriptionDate.end)).getTime()
    )
      setWeekly(false);
    else setWeekly(true);
    ...
    <table className=" w-full mt-20 mb-20 border-purple-600 rounded tableShd">
      ...
      <tbody>
        <tr>
          ...
          <td className="p-4 max-w-[17rem]">
            {weekly ? (
              <>
                {`${countdown}`}
                <br />
                <h4 className="text-base text-green-400"> Until Next Payment </h4> 
              </>
            ) : (
              <>
                <button
                  onClick={async () => {
                    ...
                    if (publicKey)
                      await updateDate(
                        ...
                        addDays(new Date(), 7).getTime().toString(),
                        publicKey.toString()
                      );
                    setWeekly(true);
                  }}
                  className="bg-green-700 hover:bg-green-800  p-4 rounded-md text-3xl  "
                >
                  PAY
                </button>
              </>
